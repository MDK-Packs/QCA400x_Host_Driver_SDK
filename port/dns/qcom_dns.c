...
A_STATUS qcom_dns_resolver(SOCKADDR_T dns_ip, char * hostname, SOCKADDR_T *host_ip, uint32_t timeout_ms)
{
    int32_t dns_sock = -1;
    ...
    int32_t result = 0;
    int32_t dns_addr_len = sizeof(dns_addr);
    ...
    do {
        ...
        dns_addr.sin_addr = dns_ip.sin_addr;
        ...
        /* Obtain WiFi context */
        QCA400x_WiFi_PTR pDev = Custom_Api_GetDriverCxt(0);
        if (NULL == pDev)
            break;

        /* Block on receive max 'timeout_ms' */
        result = t_select(pDev, dns_sock, timeout_ms);
        if (A_OK != result)
            break;
        ...
        for (tx_len = sizeof(wifi_dns_header_t); tx_len < result; tx_len++)
        {
            ...
            if (0xC0 == (uint8_t)rx_buffer[tx_len])
            {
                 ...
                 )
                {
                    ...
                    host_ip->sin_addr = dns_resp->rdata;
                    status = A_OK;
                    break;
                }
            }
